# Radial Basis Functions Recurrent Neural Network
import torch
...
##############################################
# Dataset definition (unchanged from before)
##############################################
...
##############################################
# Model definitions: RBFLayer, Base RBF-RNN, and Ensemble
##############################################

class RBFLayer(nn.Module):
    """
    A simple RBF layer that transforms the input using learned centers and a learnable log_sigma.
    """
    def __init__(self, in_features, out_features):
        super(RBFLayer, self).__init__()
        self.centers = nn.Parameter(torch.randn(out_features, in_features))
        # log_sigma is learnable (sigma = exp(log_sigma))
        self.log_sigma = nn.Parameter(torch.zeros(out_features))

    def forward(self, x):
        # x shape: (..., in_features)
        # Expand dimensions to compute pairwise Euclidean distances between x and centers.
        # x.unsqueeze(-2) shape becomes (..., 1, in_features) and centers shape is (out_features, in_features).
        diff = x.unsqueeze(-2) - self.centers  # shape: (..., out_features, in_features)
        dist_sq = torch.sum(diff ** 2, dim=-1)   # shape: (..., out_features)
        sigma = torch.exp(self.log_sigma)        # shape: (out_features,)
        # Compute the RBF activation: exp(-||x - c||^2 / (2*sigma^2))
        rbf_out = torch.exp(-dist_sq / (2 * sigma**2))
        return rbf_out

class BaseRBF_RNN(nn.Module):
    """
    A base model that applies an RBF layer to the input before processing it with a vanilla RNN.
    """
    def __init__(self, input_size=4, rbf_size=16, hidden_size=48, output_size=2, num_layers=4):
        super(BaseRBF_RNN, self).__init__()
        self.rbf = RBFLayer(input_size, rbf_size)  # Transform input features to RBF features.
        self.rnn = nn.RNN(rbf_size, hidden_size, num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, output_size)

    def forward(self, x, hidden):
        # x shape: (batch, seq_len, input_size)
        rbf_features = self.rbf(x)  # shape: (batch, seq_len, rbf_size)
        rnn_out, hidden = self.rnn(rbf_features, hidden)
        out = self.fc(rnn_out)
        return out, hidden
...
class EnsembleRBF_RNN(nn.Module):
    """
    An ensemble of BaseRBF_RNN models. The ensemble's forward pass feeds the same input into each base model,
    and the outputs are averaged.
    """
    def __init__(self, ensemble_size=3, input_size=4, rbf_size=16, hidden_size=48, output_size=2, num_layers=2):
        super(EnsembleRBF_RNN, self).__init__()
        self.ensemble_size = ensemble_size
        self.models = nn.ModuleList([
            BaseRBF_RNN(input_size, rbf_size, hidden_size, output_size, num_layers)
    ...
    def forward(self, x, hidden_list=None):
        ...
        if hidden_list is None:
            hidden_list = [model.init_hidden(x.size(0)) for model in self.models]

        outputs = []
        new_hidden_list = []
        for i, model in enumerate(self.models):
            out, new_hidden = model(x, hidden_list[i])
            outputs.append(out)
            new_hidden_list.append(new_hidden)
        # Average the outputs from all ensemble members.
        avg_output = torch.mean(torch.stack(outputs), dim=0)
        return avg_output, new_hidden_list

    def init_hidden(self, batch_size):
        return [model.init_hidden(batch_size) for model in self.models]
...
##############################################
# Training and Testing Functions (adapted to use the ensemble model)
##############################################
...
def train_model():
    ...
    # Instantiate the ensemble model (here with 3 ensemble members)
    model = EnsembleRBF_RNN(ensemble_size=3, input_size=4, rbf_size=16, hidden_size=48, output_size=2, num_layers=2).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            optimizer.zero_grad()
            # Initialize hidden states for the ensemble (a list for each model)
            hidden_list = model.init_hidden(batch_size)

            # Warm-up phase: update hidden state using the first few steps
            if seq_len > burn_in_steps:
                warm_up_input = inputs[:, :burn_in_steps, :]
                _, hidden_list = model(warm_up_input, hidden_list)
            ...
            for t in range(seq_len):
                ...
                input_t = input_t.unsqueeze(1)  # shape: (batch, 1, 4)
                output, hidden_list = model(input_t, hidden_list)
...
def test_model(model, test_datasets):
    ...
    for test_dataset in test_datasets:
        ...
        hidden_list = model.init_hidden(batch_size=1)
        if seq_len > burn_in_steps:
        ...
        for t in range(seq_len):
            ...
            with torch.no_grad():
                output, hidden_list = model(input_x, hidden_list)
